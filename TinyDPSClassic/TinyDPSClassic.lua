...
local function initialiseSavedVariables()
    ...
    if GameFontNormal then
        ...
        tdpsFont = {
            name = [[Interface\AddOns\TinyDPSClassic\Fonts\Visitor.ttf]],
            size = 13,
...
tdpsAnchor:SetBackdrop({
    bgFile = [[Interface\AddOns\TinyDPSClassic\Textures\blank.tga]],
    edgeFile = [[Interface\AddOns\TinyDPSClassic\Textures\blank.tga]],
    tile = false,
    tileSize = 1,
    edgeSize = 1,
    insets = {
...
tdpsButtonTexture:SetTexture([[Interface\AddOns\TinyDPSClassic\Textures\minimapbutton.blp]])
...
tdpsButtonTexturePushed:SetTexture([[Interface\AddOns\TinyDPSClassic\Textures\minimapbutton.blp]])
...
local UnitName, UnitGUID, UnitClass, UnitIsPlayer, UnitAffectingCombat = UnitName, UnitGUID, UnitClass, UnitIsPlayer,
    UnitAffectingCombat
-- local IsInInstance, IsInRaid, IsInGroup, InCombatLockdown, IsInBattle = IsInInstance, IsInRaid, IsInGroup, InCombatLockdown, C_PetBattles.IsInBattle
...
local function isPvPZone()
    ...
    if instanceType == "pvp" or instanceType == "arena" or (uiMapID == 123 and isActiveWintergrasp) or
        (uiMapID == 244 and isActiveTolBarad) or (uiMapID == 588 and isActiveAshran) then
        return true
    end
end
...
local function report(button, channel, playerName)
    ...
    for i = 1, math.min(#report, tdpsReportLength) do
        if report[i].n > 0 then
            SendChatMessage(format("%i. %s    %i    %i%%    (%i)", i, report[i].name, report[i].n,
                report[i].n / tdpsFight[tdpsF][tdpsV] * 100, report[i].n / report[i].t), channel, nil, destination)
        end
    end
end

local function visibilityEvent()
    if (tdps.hidePvP and isPvPZone()) or (tdps.hideBattle and IsInBattle()) or (tdps.hideSolo and not IsInGroup()) or
        (tdps.hideOOC and not UnitAffectingCombat("player")) or (tdps.hideIC and UnitAffectingCombat("player")) then
        tdpsFrame:Hide()
    else
        tdpsFrame:Show()
        tdpsRefresh()
...
local function startNewFight(target, GUID)
    ...
    if tdpsFight[2].d + tdpsFight[2].h > 0 and
        ((tdps.onlyBossSegments and tdpsFight[2].boss) or not tdps.onlyBossSegments) then
        tinsert(tdpsFight, 2, {
            name = target or "?",
...
local function ver()
    echo(tdpsL.helpVersion .. " " .. GetAddOnMetadata("TinyDPSClassic", "Version") ..
             " by Sideshow (formerly) and Talyrius")
end
...
function SlashCmdList.TINYDPS(msg, editbox)
    ...
    elseif cmd == "visiblebars" and tonumber(arg) then
        tdpsVisibleBars = min(40, max(1, tonumber(arg)))
        scrollPos = 1
        tdpsRefresh()
...
tdpsDropDown.initialize = function(self, level)
    if level == 1 then
        PlaySound(SOUNDKIT.GS_TITLE_OPTION_EXIT)
        newBu(level, "TinyDPSClassic       ", 1, 1)
    ...
    elseif level == 2 and UIDROPDOWNMENU_MENU_VALUE == "fight" then
        ...
        newBu(level, tdpsL.current .. "    " .. (tdpsFight[2].name or tdpsL.empty), nil, nil, nil, nil, nil,
            changeFight, 2, nil, checkFight(2))
        if tdpsNumberOfFights > 2 then
            newBu(level, "", nil, 1, nil, nil, nil, nil, nil, nil, nil, 1)
        end
        for i = 3, tdpsNumberOfFights do
            newBu(level, format("%s %i     %s", tdpsL.fight, i - 2, (tdpsFight[i].name or tdpsL.empty)), nil, nil, nil,
                nil, nil, changeFight, i, nil, checkFight(i))
        end
        newBu(level, "", nil, 1, nil, nil, nil, nil, nil, nil, nil, 1)
    ...
    elseif level == 3 and UIDROPDOWNMENU_MENU_VALUE == "various" then
        ...
        newBu(level, tdpsL.minimapButton, nil, nil, nil, nil, 1, toggleMinimapButton, nil, nil, tdps.showMinimapButton,
            nil, 1)
    ...
    elseif level == 4 and UIDROPDOWNMENU_MENU_VALUE == "font" then
        if GetLocale() == "koKR" then
            ...
            newBu(level, "데미지 글꼴", nil, nil, nil, nil, nil, changeFont, "font", [[Fonts\K_Damage.TTF]],
                function()
                    if tdpsFont.name == [[Fonts\K_Damage.TTF]] then
                        return true
                    end
                end, nil, nil)
            newBu(level, "퀘스트 글꼴", nil, nil, nil, nil, nil, changeFont, "font", [[Fonts\K_Pagetext.TTF]],
                function()
        ...
        elseif GetLocale() == "zhTW" then
            ...
            newBu(level, "Visitor", nil, nil, nil, nil, nil, changeFont, "font",
                [[Interface\AddOns\TinyDPSClassic\Fonts\visitor.ttf]], function()
                    if tdpsFont.name == [[Interface\AddOns\TinyDPSClassic\Fonts\visitor.ttf]] then
                        return true
                    end
                end, nil, nil)
            ...
            newBu(level, "Friz Quadrata TT", nil, nil, nil, nil, nil, changeFont, "font", [[Fonts\FRIZQT__.TTF]],
                function()
        ...
        newBu(level, "Custom", nil, nil, nil, nil, nil, changeFont, "font",
            [[Interface\AddOns\TinyDPSClassic\Fonts\custom.ttf]], function()
                if tdpsFont.name == [[Interface\AddOns\TinyDPSClassic\Fonts\custom.ttf]] then
                    return true
                end
            end, nil, nil)
    elseif level == 4 and UIDROPDOWNMENU_MENU_VALUE == "layout" then
        ...
        newBu(level, tdpsL.rank, nil, nil, nil, nil, 1, function()
            tdps.showRank = not tdps.showRank
            tdpsRefresh()
...
local function newBar(g)
    ...
    dummybar:SetBackdrop({
        bgFile = [[Interface\AddOns\TinyDPSClassic\Textures\wglass.tga]],
        edgeFile = [[Interface\AddOns\TinyDPSClassic\Textures\blank.tga]],
        tile = false,
        tileSize = 1,
        edgeSize = 1,
    ...
    dummybar:SetStatusBarTexture([[Interface\AddOns\TinyDPSClassic\Textures\wglass.tga]])
    ...
    dummybar:SetScript("OnEnter", function(self)
        ...
        if petAmount > 0 then
            GameTooltip:AddDoubleLine(tdpsL.byPets, petAmount .. " (" .. round(petAmount / (self.n) * 100, 0) .. "%)",
                1, 1, 1, 1, 1, 1)
        end
        ...
        if tdps.trackSpells then
            ...
            for k, v in pairs(tdpsPlayer[g].fight[tdpsF][tdpsV .. "s"]) do
                for kk, vv in pairs(v) do
                    ttSpellMerge[k] = (ttSpellMerge[k] or 0) + vv
                    ttMobMerge[kk] = (ttMobMerge[kk] or 0) + vv
                end
            end
            for i = 1, #pet do
            ...
            for i = 1, tdps.tooltipTargets do
                if ttSort[i] then
                    GameTooltip:AddDoubleLine(i .. ". " .. ttSort[i], ttMobMerge[ttSort[i]] .. " (" ..
                        round(ttMobMerge[ttSort[i]] / (self.n) * 100, 0) .. "%)", 1, 1, 1, 1, 1, 1)
                end
            end
            wipe(ttSort)
            wipe(ttSpellMerge)
...
local function tdpsCombatEvent(self, event, ...)
    ...
    if not isValidEvent[event] or strsplit("-", sourceGUID) == "Vehicle" or
        (band(destFlags, 16) > 0 and isDamage[event]) or (band(destFlags, 16) == 0 and isHeal[event]) or arg15 ==
        "EVADE" then
        return
    end
...
tdpsFrame:SetScript("OnEvent", function(self, event)
    local addonVer = GetAddOnMetadata("TinyDPSClassic", "Version")
    ...
    cColor = setmetatable(cColor, {
        __index = CUSTOM_CLASS_COLORS or RAID_CLASS_COLORS
    })
...
tdpsButtonFrame:SetScript("OnEnter", function(self)
    ...
    GameTooltip:SetText("TinyDPSClassic")
    ...
    -- personal amount
    local ownAmount, ownTime, pet = 0, 0, 0

    if tdpsPlayer[UnitGUID("player")] then
        pet, ownAmount, ownTime = tdpsPlayer[UnitGUID("player")].pet,
            tdpsPlayer[UnitGUID("player")].fight[tdpsF][tdpsV], tdpsPlayer[UnitGUID("player")].fight[tdpsF].t
        for i = 1, #pet do
        ...
        if ownAmount > 0 then
            if (band(tdps.layout, 1) == 1) then -- short format bit active
                GameTooltip:AddDoubleLine(UnitName("player"),
                    format("%s (%s)", short(ownAmount), short(ownAmount / ownTime)), 1, 1, 1, 1, 1, 1)
            else
                GameTooltip:AddDoubleLine(UnitName("player"), format("%i (%i)", ownAmount, ownAmount / ownTime), 1, 1,
                    1, 1, 1, 1)
            end
        end
    end
    ...
    if partyAmount > ownAmount then
        if (band(tdps.layout, 1) == 1) then -- short format bit active
            GameTooltip:AddDoubleLine(tdpsL.raid, format("%s (%s)", short(partyAmount), short(partyAmount / partyTime)),
                1, 1, 1, 1, 1, 1)
        else
            GameTooltip:AddDoubleLine(tdpsL.raid, format("%i (%i)", partyAmount, partyAmount / partyTime), 1, 1, 1, 1,
                1, 1)
        end
    end
